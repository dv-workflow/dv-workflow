...
- Trong **Plan**: mục **Góc nhìn & Trade-off** — với mỗi quyết định lớn (nên có pros vs cons ?), ghi ngắn tác động lên user / admin / dev / security.
...
- ""Khi làm task nên áp dụng kết hợp tư duy trong `.dev-workflow/skills/THINKING.md`: xem xét rủi ro và edge case, tác động lên các module khác, và ít nhất 2 góc nhìn (vd: user + security). Nếu phát hiện giả định sai hoặc thiếu, ghi vào Notes/Changelog của task.""
